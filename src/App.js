...
class App extends React.Component {
  state = {
    ...
    on: {foot: 0, search: 0, feed: 1, bgFlur: 0}, // 各组件开关
  };
  ...
  handleGlobalKey = (e) => {
    ...
    // 右键 显示左栏热榜
    if(e.keyCode===39){
      let obj = Object.assign({}, this.state.on, {foot: 0, search: 0});
      this.setState({on: obj});
    // 左键 隐藏热榜
    }else if(e.keyCode===37){
      let obj = Object.assign({}, this.state.on, {foot: 1, search: 1});
      this.setState({on: obj});
  ...
  render() {
    ...
    return (
      <div className="main" onKeyDown={e=>{this.handleGlobalKey(e)}}>
        <div className="cover">  
          <img className={this.state.on.bgFlur?'imgBg img-filter': 'imgBg'} alt="背景图片" src={bgImg? this.state.bgImg: this.state.bgSrc}></img>
        </div>
        ...
        <SearchBar  isOn={obj=>this.isOn(obj)} setState={data=>{this.setState(data)}} style={{visibility: on.search?'visible':'hidden'}}/>
