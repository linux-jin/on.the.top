...
function SVGNews(props) {
  const handleOpenFeed = () => {
    props.setSettingOn(false);
    props.isOn({ search: 0, foot: 0, feed: 1, menu: 0, tool: 0});
  };
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="svgNews svgMenu" onClick={handleOpenFeed}>
      <g>
...
function SVGSetting(props) {
    ...
    const handleClickSetting = () => {
        if(props.isSettingOn){
            props.isOn({tool: 0});
            props.setSettingOn(false);
        }
        else{
            props.isOn({tool: 1});
            props.setSettingOn(true);
        }
    }
  return (
    <svg viewBox="0 0 24 24" aria-hidden="true" className="svgMenu svgSetting" onClick={handleClickSetting} style={{background: props.isSettingOn?"rgba(255,255,255,.3)":"rgba(255,255,255,.1)"}}>
      <g>
...
export function MenuBar(props) {
  ...
  return (
    <div className="MenuBar" style={props.style}>
      ...
      <SVGSetting isOn={props.isOn} setSettingOn={setSettingOn} isSettingOn={isSettingOn} />
    </div>
  );
}
